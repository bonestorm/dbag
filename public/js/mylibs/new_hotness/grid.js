...
*/
  ...
  */
  ...
  OBJ.update_cursor = function(){
  
    var objs = _globs.grid.get_objects(_cx,_cy);
  
    if(_globs.main.shiftKey && objs.length == 0){
      _globs.main.change_cursor("move");
    } else {
      if(!_globs.main.shiftKey && objs.length == 0){
        _globs.main.change_cursor("pointer");
      } else {
        _globs.main.change_cursor("auto");
      }
    }
...
//    } else {
      ...
      if(_globs.main.altKey || _globs.main.ctrlKey){
        _drag_type = "window";
      } else {
...
//    OBJ.update_cursor();
  ...
  //this handles the event of someone picking 'new_table' from the menu
  //not used right now!
  OBJ.new_table = function(option,hook){
  
    //get new table name
    var new_table_prefix = "new_pable_";
    var default_table_name = new_table_prefix+TABLE.new_table_id;
    TABLE.new_table_id++;
    default_table_name += " ";
    for(var i = 0;i < Math.floor((Math.random()*5)+1);i++){
      default_table_name += "x";
    }
    var new_table_name=prompt("new table name",default_table_name)

	
    //table(id,name,hook)//id is the same as the name for now
    var _new_table = new TABLE(_globs,{cx: hook.cx,cy: hook.cy,name: new_table_name});
    if(_new_table.error != undefined && _new_table.error){
	  //log('failed');
	  //failed.  possibly something in the way
    } else {
      OBJ.add_obj(_new_table);
      _globs.refresh();
    }
